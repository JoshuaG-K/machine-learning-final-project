...
public class PCA {
    ...
    private static double[] getEigenValues(double[][] matrix) throws IOException, InterruptedException {
        Process process = new ProcessBuilder("python", "EigenValueCalculator.py").start();

        MatrixIO.writeMatrix(process.getOutputStream(), matrix);
    ...
    private static double[][] getEigenVectors(double[][] matrix) throws IOException, InterruptedException {
        ...
        MatrixIO.writeMatrix(process.getOutputStream(), matrix);
        process.getOutputStream().close();  // Close the input stream to signal the end of input
